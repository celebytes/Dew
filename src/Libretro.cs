...
namespace Dew
{
    sealed class Libretro
    {
        ...
        public string Simulate(int duration, uint input, int pressDuration)
        {
            ...
            if(Settings.DestroyGIFs) File.Delete(Program.AbsolutePath + $"/GIFs/{state}.gif");
            if(Settings.DestroyStates) File.Delete(Program.AbsolutePath + $"/libretro/saves/{Settings.Core}/{Settings.RomName}/save_{state}.state");
